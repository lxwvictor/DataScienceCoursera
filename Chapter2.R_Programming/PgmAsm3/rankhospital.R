rankhospital <- function(state, outcome, num = "best") {
        ...
        names(hospitalDF)[2] <- paste("Rate")
        hospitalDF <- cbind(hospitalDF, data.frame(Rank = seq(nrow(hospitalDF))))
        
        if(num == "best") num <- 1
        else if(num == "worst") num <- nrow(hospitalDF)
        else if(num > nrow(hospitalDF)) return(NA)
