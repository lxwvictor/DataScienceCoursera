...
Before running into any calculation, let's take a look at the data in text and plot. (Refer to appendix 1)
...
Assume the null hypothesis is that auto and manual transmission are equally efficient. (Refer to appendix 2)
...
Let's take a look at what will happen if I choose all columns (other than MPG) as regressors. (Refer to appendix 3)

Look at those p-values, the regressors all look equally unsignificant. It's quite difficult to choose which regressor or not. We can try eliminate the regressor one by one. But as I mentioned, I prefer to leave it to computer and algorithm to determine for me. (Refer to appendix 4)
...
Before concluding this _reduced.model_ is the final selection, let's do a comparison with the _full.model_ and also plot the residual to ensure there is no pattern or correlation. (Refer to appendix 5)
...
The F-statistic is 52.75, much greater than 1, R-squared is 0.83 which is close to 1. These figures all agree the _reduced.model_ is a "fitted" model. And lastly let's plot the model to perform some diagnostics. (Refer to appendix 6)
